...
echo "define ( 'WP_SITEURL', 'http://$IP_BALANCER/wordpress' );" >> /var/www/html/wordpress/wp-config.php
echo "define ( 'WP_HOME', 'http://$IP_BALANCER' );" >> /var/www/html/wordpress/wp-config.php
sed -i 's/database_name_here/wordpress/' /var/www/html/wordpress/wp-config.php
sed -i 's/username_here/wpuser/' /var/www/html/wordpress/wp-config.php
sed -i 's/password_here/wpuser/' /var/www/html/wordpress/wp-config.php
sed -i 's/localhost/18.207.209.185/' /var/www/html/wordpress/wp-config.php
...
sed -i '/LOGGED_KEY/d' /var/www/html/wordpress/wp-config.php
...
echo "/var/www/html/wordpress     $IP_SLAVE(rw,sync,no_root_squash,no_subtree_check)" > /etc/exports
